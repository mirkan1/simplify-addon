...
class MessageBoxOperator(bpy.types.Panel):
    ...
    bl_region_type = 'UI'
    bl_category = 'Selective Simplify'
            ...
            for i in nodes: 
                ...
                if tur == "<class 'bpy.types.ShaderNodeTexImage'>":
                    ...
                    if image.generated_width + image.generated_height  < res_ + res_:
                        if j.name not in error_arr:
                            self.report({"ERROR"}, f"{j.name}'s dimension is lesser than given texture size")
                            error_arr.append(j.name)
                        continue # return {'CANCELLED'} 
...
class Non_Selected_ImageResise(bpy.types.Operator):
    bl_idname = "object.image_non_selected_resise"
    bl_label = "resize an image and save its value for a further call"
